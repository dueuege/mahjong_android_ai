...
- [ ] **Auto-mic can crash the system ASR on locked screen.** Observed `com.xiaomi.mibrain.speech` OOM (pthread_create) when `coachAudioAuto` fired while the device was locked. Not our crash, but we shouldn't start the mic when the activity isn't actually resumed/visible. Gate `startMic()` on a `Lifecycle.State.RESUMED` check.
- [ ] **140 MB debug APK.** The bundled fp32 `tiles.onnx` (66 MB) + ORT native libs across ABIs balloon the APK. For a shippable build: int8-quantize the model (`model.export(format='onnx', int8=True)`, ~1/4 size) and `abiFilters` down to arm64-v8a only.
