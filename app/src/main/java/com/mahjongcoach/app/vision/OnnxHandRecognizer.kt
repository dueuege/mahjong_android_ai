...
class OnnxHandRecognizer(
    ...
    modelAsset: String? = null,   // null = auto-discover the first *.onnx asset
    ...
    init {
        val asset = modelAsset ?: findModelAsset(context) ?: MODEL_ASSET
        val bytes = context.assets.open(asset).use { it.readBytes() }
        // Try NNAPI (hardware accel) first; some YOLO graphs have ops NNAPI
        // can't partition (e.g. Split), which fails at createSession — fall
        // back to plain CPU in that case.
        session = runCatching {
            ...
            Log.w(TAG, "NNAPI session failed (${it.message}); falling back to CPU")
            val cpu = OrtSession.SessionOptions().apply { setIntraOpNumThreads(2) }
            env.createSession(bytes, cpu)
        }
        ...
        val customMeta = runCatching { session.metadata.customMetadata }.getOrNull()
        Log.i(TAG, "model metadata keys=${customMeta?.keys} names=${customMeta?.get("names")?.take(400)}")
        val namesRaw = customMeta?.get("names")
    ...
    override fun recognize(bitmap: Bitmap): IntArray? {
        ...
        if (!snapPending && !cooled) return null
        snapPending = false
        lastFiredAt = now
    ...
    private fun mapFromNames(raw: String): IntArray? {
        ...
        // A numbered tile is exactly one suit-letter + one rank-digit, in either
        // order ("b1" or "1b"). Two-letter honors (ew/gd/...) and longer names
        // parse to null and are dropped.
        fun parseTile(n: String): Pair<Char, Int>? {
            if (n.length != 2) return null
            val a = n[0]; val b = n[1]
            return when {
                a.isLetter() && b.isDigit() -> a to (b - '0')
                a.isDigit() && b.isLetter() -> b to (a - '0')
                else -> null
            }
        }
        // Max rank per suit letter: a real numbered suit has ranks up to 9;
        // flowers/seasons only go to 4 — so max≥5 marks an actual suit.
        val maxRank = HashMap<Char, Int>()
        names.values.forEach { n -> parseTile(n)?.let { (p, r) -> maxRank[p] = maxOf(maxRank[p] ?: 0, r) } }

        // Suit identity by letter. Covers the common conventions:
        //  man 万 = m/w(wan)/c(characters); pin 筒 = p/t(tong)/d(dots);
        //  sou 条 = s(sou)/b(bamboo). Bonus 's' (season) never reaches here
        //  because its maxRank < 5.
        ...
        var mapped = 0
        for ((idx, n) in names) {
            val (p, rank) = parseTile(n) ?: continue
            if ((maxRank[p] ?: 0) < 5) continue        // flowers/seasons → drop
            val base = suitBase(p) ?: continue
            if (rank in 1..9) { map[idx] = base + (rank - 1); mapped++ }
        }
        return if (mapped >= 9) map else null
    }
    ...
    companion object {
        ...
        /** First `*.onnx` file shipped in assets, or null if none. */
        fun findModelAsset(context: Context): String? = runCatching {
            context.assets.list("")?.firstOrNull { it.endsWith(".onnx", ignoreCase = true) }
        }.getOrNull()

        /** True iff some on-device model is shipped in this APK. */
        fun isAvailable(context: Context): Boolean = findModelAsset(context) != null
